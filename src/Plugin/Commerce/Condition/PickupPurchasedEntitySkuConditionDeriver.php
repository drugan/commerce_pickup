 ...
 */
class PickupPurchasedEntitySkuConditionDeriver extends DeriverBase implements ContainerDeriverInterface {
  ...
  public function getDerivativeDefinitions($base_plugin_definition) {
    ...
    foreach ($purchasable_entity_types as $purchasable_entity_type_id => $purchasable_entity_type) {
      if ($base_plugin_definition['entity_type'] === 'commerce_order') {
        $display_label = new TranslatableMarkup('Order contains specific shippable :item SKU', [':item' => $purchasable_entity_type->getPluralLabel()]);
      }
      else {
        $display_label = new TranslatableMarkup('Specific shippable :item SKU', [':item' => $purchasable_entity_type->getSingularLabel()]);
      }
